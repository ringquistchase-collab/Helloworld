...
DigitalDNA fingerprint actually changed (grew) from mining activity, and
...
async def main():
    ...
    dna_store = os.path.join(WORKDIR, "identity.json")
    dna = DigitalDNA(owner_id="chase-allen-ringquist", store_path=dna_store)
    initial_fingerprint = dna.build_fingerprint_dna()
    # Frozen ONCE, before any mining: this is what every block broadcasts
    # and every peer verifies against for the whole run. dna.signals keeps
    # growing separately from real mining activity (see network_node.py).
    identity_strand = initial_fingerprint
    print(f"\nIdentity strand carried/verified by every node this run: {identity_strand}")
    print(f"Starting signal count: {len(dna.signals)}\n")
    ...
    nodes = [
        NetworkNode(
            node_id=i,
            port=p,
            peers=[("127.0.0.1", q) for q in ports if q != p],
            dna=dna,
    ...
    final_fingerprint = dna.build_fingerprint_dna()
    print(f"\nIdentity strand used for verification all run (frozen by design): {identity_strand}")
    print(f"DigitalDNA's LIVE fingerprint now (after real mining signals): {final_fingerprint}")
    print(f"Final signal count: {len(dna.signals)}")
    print(f"Live fingerprint diverged from the frozen broadcast one (expected, by design): "
          f"{final_fingerprint != initial_fingerprint}")
